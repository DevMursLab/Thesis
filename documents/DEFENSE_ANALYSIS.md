...
Three mechanistically independent methods (attention rollout, gradient × input, leave-one-out occlusion) all identify the same top modality (audio) **on the dev split**. For the facial-feature attribution specifically: gradient×input saliency's actual top-ranked AU is **AU20 (Lip Stretcher)**, not AU04 — a fact verified by directly re-running `src/explainability/explain_fusion.py` this session. AU04 (Brow Lowerer, the clinically-established depression marker) ranks **5th** of 20 channels (importance 0.079 vs. AU20's 0.130). The paper was corrected this session to report this honestly (AU04 "within the top-5," not "the top feature") rather than the earlier, factually wrong claim. Do NOT say "AU04 is the top feature" in the viva — say "AU04 is within the top-5 attributed features; the single top-ranked feature is AU20, which does not have the same established clinical literature link." Separately, a test-time missing-modality-masking experiment (Section VIII-B) shows the dev-split modality ranking does *not* fully predict test-time necessity: masking audio does not hurt test performance, while masking text collapses test AUC from 0.706 to 0.435. Be ready to explain both distinctions plainly — these are genuinely interesting findings, not contradictions to hide.
...
First, a factual correction to have ready: AU04 is not this method's single top-ranked feature — gradient saliency's top-ranked channel is actually AU20 (Lip Stretcher); AU04 ranks 5th of 20. With that said: gradient saliency is a post-hoc attribution method regardless of rank — the model was never told to look at any specific AU. Whatever ranking emerges (AU20 first, AU04 fifth) emerges because those channels' variation correlated with PHQ-8 labels during training. AU04's presence in the top-5, even at 5th rather than 1st, is still a data-driven finding that partially matches independent clinical literature (Ekman's FACS work on AU04/Brow Lowerer as a sadness/distress marker). AU20 (the actual top feature) does not have the same established clinical link in the literature we reviewed, which is itself worth acknowledging honestly rather than glossing over — it may reflect a genuine alternative signal, a dataset-specific artifact, or noise from evaluating on only 34 dev participants. Circularity would only exist if AU04 had been hand-selected as a feature *because* it's a known marker; instead all 20 AUs are used equally and the ranking is read off the gradient, wherever it points.
...
XAI (dev): audio attention weight 0.39 (top modality); face-feature saliency
  top-ranked = AU20 (Lip Stretcher, 0.130), AU04 (Brow Lowerer) ranks 5th
  (0.079) -- AU04 is in the top-5 but is NOT the single top feature, correct
  this if asked. Also: test-time masking shows audio is NOT most necessary
...
5. **Modality dropout is implemented at batch level, not per-example** (`src/models/multitask_fusion.py:_apply_modality_dropout` draws one Bernoulli per modality per mini-batch, so all examples in a batch share the same drop/keep decision). The paper (Section on Modality Dropout) now describes this accurately, but it is a weaker regularizer than fully independent per-example masking would be, and plausibly contributes to the mixed (not uniformly graceful) test-time robustness result. Fixing it to per-example masking would require a full retrain of the proposed model and rerunning every downstream script that depends on the checkpoint — estimated ~1 day of work, not attempted this session because it would invalidate and require regenerating essentially every headline number in the paper under a tight defense timeline.
6. **Modality feature arrays are merged positionally, not by participant ID** (`src/training/train_multitask.py:load_split`, and similarly in other training scripts) — face/audio/text preprocessing pipelines have different, uncoordinated per-participant skip criteria, and nothing currently enforces that position `k` in each array refers to the same participant. Verified this session (by decoding participant IDs from `meta_train.csv`/`meta_dev.csv`/`gender_*.csv`) that the three pipelines happen to skip identically for the current committed data, so today's numbers are not corrupted — but this is a silent-failure risk if the raw data or preprocessing thresholds ever change and the pipelines are rerun independently. If asked "how do you know your features and labels are correctly paired," the honest answer is "verified by participant-ID cross-check this session, but not enforced by an assertion in the code" — a fair thing to flag as a reproducibility hardening item rather than a current-results problem.
7. **A stale/inconsistent `au_saliency` field exists inside `results/metrics/phase6_explainability.json`** (a Python dict-key collision from `AU04_r`/`AU04_c`, `AU12_r`/`AU12_c`, `AU15_r`/`AU15_c` both mapping to the same stripped name after `_r`/`_c` suffix removal, so the summary dict silently overwrites one duplicate with the other). The `top_action_units` list used for the paper's actual reported ranking is unaffected by this bug (it indexes by raw channel position, not by name), and was verified this session by directly re-running `explain_fusion.py` end-to-end. Only the auxiliary full-ranking dict in the JSON is affected; not something the paper currently relies on, but worth fixing if that JSON is ever used directly again.
